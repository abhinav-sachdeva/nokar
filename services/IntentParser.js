...
exports.getSlots = intent =>{
    ...
    if(intent.name.indexOf("AMAZON")<0){
        ...
        mappings.getSlots(intent.name).map(slotName => {
            if(intent.slots[slotName].resolutions.resolutionsPerAuthority[0].status.code==="ER_SUCCESS_MATCH"){
                parsedSlot.slotDetail = intent.slots[slotName].value;
                let slotCategories = 
                    intent.slots[slotName].resolutions.resolutionsPerAuthority[0]
                    .values.map(category => category.value.name)
        ...
        parsedSlot = {
            ...
            slotDetail:"default",
            endSession: (intent.name.indexOf("CancelIntent")>0 || intent.name.indexOf("StopIntent")>0)
        }
        console.log("parsedSlot: ",parsedSlot)
    }
    return parsedSlot;
